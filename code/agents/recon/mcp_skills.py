...
async def call_mcp_skill(channel: aio_pika.Channel, tool_name: str,arguments: dict,) -> str:
    ...
    logger.info("[MCP] Publicando tool=%s correlation_id=%s arguments=%s",tool_name,correlation_id,log_preview(arguments),)
    ...
    try:
        ...
        logger.info("[MCP] Respuesta recibida tool=%s correlation_id=%s is_error=%s",tool_name,correlation_id,response.get("result", {}).get("isError", False),)

        is_error = response.get("result", {}).get("isError", False)
        content = response.get("result", {}).get("content", [])
        raw_output = content[0].get("text", "") if content else json.dumps(response)

        logger.info("[MCP] RAW INFO: %s", raw_output)
...
async def get_mcp_catalog(channel: aio_pika.Channel) -> list:
    ...
    try:
        ...
        logger.info( "[MCP] Catálogo recibido correlation_id=%s tools=%d names=%s",correlation_id,len(tools),[tool.get("name") for tool in tools],)
